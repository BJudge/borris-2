...
shared_examples_for BikeContainer do
  it 'has a default capcity when initialized' do
    expect(subject.capcity).to eq BikeContainer::DEFAULT_CAPACITY
  end

    describe 'capacity' do
      it 'can be overridden on initialize' do
        random_capcity = Random.rand(100)
        subject = described_class.new random_capcity
        expect(subject.capacity).to eq random_capcity
      end
    end
